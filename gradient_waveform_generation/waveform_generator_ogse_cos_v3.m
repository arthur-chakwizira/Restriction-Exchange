%Random generation of cosine-modulated OGSE waveforms
tic
close all
% dock_figures(1)
...
wps.slew_max = 700; %maximum slew rate in Tesla/seconds
...
for c_wf =  1:Nwfs
    ...
    reject_cp = true;
    while reject_wf
        wps.T = 16e-3+ randi([0 235])*1e-3; %total encoding time in seconds %%This needs to be changed as well!!!!!!!!
        ...
        n_lobes = randi([2 20]); %quicker than above
        
        if mod(n_lobes, 2) ~= 0; continue; end
        
        %get pulse
        pulse = t_pulse*0;
        period = 2*max(t_lobe)/n_lobes;
        tmp_pulse = wps.g_max*cos((2*pi/period)*t_lobe);
        
        %do sine apodisation
        rise_t = 0:dt:(0.25*period);
        n_4 = numel(rise_t);
        rise_period = 2*max(rise_t);
        rise_g = wps.g_max*sin((2*pi/rise_period)*rise_t);
        
        tmp_pulse(1:n_4) = rise_g;
        tmp_pulse(end-n_4+1:end) = rise_g;
