function R_updated = resex_mc_spin_reflect_12(sim_data, opt)
...
rejected_from_1 = sim_data.rejected_from_1;


if ~any(rejected_from_1); R_updated = []; return; end

V0 = R_old(rejected_from_1,:); %starting points
V1 = R_old_moved(rejected_from_1,:); %end points
...
Ventry = zeros(sum(rejected_from_1),2); %will contain points of entry into intracellular

for row = 1:sum(rejected_from_1)
...
V_final = Ventry - d_in_1.*Vn; %determine correct final positions outside cell

failed = (V_final(:,1)-c_x).^2 + (V_final(:, 2)-c_y).^2 <= r^2; %this is very rare
