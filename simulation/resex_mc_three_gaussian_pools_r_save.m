function resex_mc_three_gaussian_pools_r_save(r_fn, opt)
%This function simulates diffusion in 3 Gaussian compartments
...
prog = 0;
fprintf(1,'Running simulation (three Gaussian pools): %3d%%\n',prog);
%% Retrieve parameters
...
D_0 = opt.sim_opt.D_0; %3x1 array

n_dim = opt.sim_opt.n_dim; %number of dimensions, 2

sig_ma = zeros(N,1);
first = 1:floor(N/3); second = (floor(N/3)+1):2*floor(N/3); 
third = (numel(first)+numel(second)+1):N;

sig_ma(first) = sqrt(2*D_0(1)*sim_dt); %standard dev of steps in intracellular; 3x1
sig_ma(second) = sqrt(2*D_0(2)*sim_dt); %standard dev of steps in intracellular; 3x1
sig_ma(third) = sqrt(2*D_0(3)*sim_dt); %standard dev of steps in intracellular; 3x1

R_old = randn(N, n_dim).*sig_ma; % initial positions
